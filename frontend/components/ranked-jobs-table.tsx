...
import InternListJobsPanel from "@/components/intern-list-jobs-panel";
...
type JobsSourceTab = "jobspy" | "internlist";
...
function parseJobsTab(raw: string | null | undefined): JobsSourceTab | null {
  if (raw === "internlist" || raw === "intern-list" || raw === "intern_list") return "internlist";
  if (raw === "jobspy" || raw === "ranked") return "jobspy";
  return null;
}
...
export default function RankedJobsTable({
  ...
  const [jobsTab, setJobsTab] = useState<JobsSourceTab>(() => {
    return parseJobsTab(searchParams.get("tab")) || "jobspy";
  });
  ...
  useEffect(() => {
    try {
      ...
      const fromUrl = parseJobsTab(searchParams.get("tab"));
      if (fromUrl) {
        setJobsTab(fromUrl);
      } else {
        const savedTab = parseJobsTab(localStorage.getItem("resume-agent-jobs-tab"));
        if (savedTab) setJobsTab(savedTab);
      }
    } catch {
      /* ignore */
    }
    ...
    setSourceReady(true);
    setTabReady(true);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- hydrate prefs once on mount
  }, []);

  // Only follow the URL when the query string itself changes (browser back/forward
  // or external navigation). Do NOT depend on `jobsTab` here — otherwise clicking
  // JobSpy briefly races with the still-stale `?tab=internlist` and snaps back.
  useEffect(() => {
    const fromUrl = parseJobsTab(searchParams.get("tab"));
    if (fromUrl) setJobsTab(fromUrl);
  }, [searchParams]);

  useEffect(() => {
    if (!tabReady) return;
    try {
      localStorage.setItem("resume-agent-jobs-tab", jobsTab);
    } catch {
      /* ignore */
    }
    const params = new URLSearchParams(searchParams.toString());
    // Always write an explicit tab so JobSpy is not represented by "missing param"
    // (which previously made URL→state sync unable to distinguish intent).
  ...
  }, [jobsTab, tabReady, router, searchParams]);
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/v1/jobs/sources/list?user_id=${encodeURIComponent(getAuthUserId())}`)
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f4] text-slate-900" data-testid="ranked-jobs-page">
      <AppTopNav active="jobs" displayName={displayName} onLogout={onLogout} />
      <header className="border-b border-slate-200/80 bg-white/95">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-6 py-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-950">
              {jobsTab === "internlist" ? "Intern-list Jobs" : "Ranked Jobs"}
            </h1>
            <p className="mt-0.5 text-xs text-slate-500">
              {jobsTab === "internlist"
                ? "Intern-list.com scrape viewer — select jobs for Shopping Cart / Tailor."
                : "JobSpy / provider discovery — Match = resume↔JD heuristic. Click a row for Match, then JD."}
            </p>
          </div>
          <FlowStepper
            current="jobs"
        ...
        <div className="mx-auto flex max-w-7xl px-6 pb-3">
          <div
            className="inline-flex rounded-xl bg-slate-100 p-1 text-xs font-semibold"
            role="tablist"
            aria-label="Jobs source"
            data-testid="jobs-source-tabs"
          >
            <button
              type="button"
              role="tab"
              aria-selected={jobsTab === "jobspy"}
              data-testid="jobs-tab-jobspy"
              onClick={() => setJobsTab("jobspy")}
              className={`rounded-lg px-3.5 py-1.5 transition ${
                jobsTab === "jobspy" ? "bg-white text-slate-950 shadow-sm" : "text-slate-500 hover:text-slate-800"
              }`}
            >
              JobSpy / Ranked
            </button>
            <button
              type="button"
              role="tab"
              aria-selected={jobsTab === "internlist"}
              data-testid="jobs-tab-internlist"
              onClick={() => setJobsTab("internlist")}
              className={`rounded-lg px-3.5 py-1.5 transition ${
                jobsTab === "internlist" ? "bg-white text-slate-950 shadow-sm" : "text-slate-500 hover:text-slate-800"
              }`}
            >
              Intern-list
            </button>
      ...
      <div className={`mx-auto px-6 py-4 ${jobsTab === "internlist" ? "max-w-[1400px]" : "max-w-7xl"}`}>
        {jobsTab === "internlist" ? (
          <InternListJobsPanel />
        ) : (
          <>
