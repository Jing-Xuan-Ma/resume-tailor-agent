...
def assess_listing_quality(lead: dict[str, Any], *, min_chars: int = 500) -> dict[str, Any]:
    ...
    # Extension / closed Jobright posts often lack an ATS Apply link — page URL is enough to import for tailor.
    if not url or not url.startswith("http"):
        for key in ("jobright_url", "page_url", "apply_url"):
            alt = str(meta.get(key) or lead.get(key) or "").strip()
    ...
    # "Unknown Company" is the extension placeholder when DOM company node is missing — still allow import.
    if not company or company.lower() in {"nan", "none", "n/a"}:
